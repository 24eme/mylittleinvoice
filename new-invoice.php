<?php
  ...
  <body class="skin-blue sidebar-mini">
    <div class="wrapper">
      ...
      <?php 
	      $horizontal_menu = '<li><a href="#" onclick="print_pt();return false;">Produit types</a></li>';
      ...
      <?php include('include/left.php'); ?>
       ...
       <?php
	     ...
	     global $bd;
	     global $lang;
	     // Chargement des infos de la facture de base pour la duplication
	     if(!empty($_GET['duplicate_invoice'])){
		     $p = $bd->get_results('SELECT * FROM ' . $bd->prefix . 'invoices WHERE id="' . $_GET['duplicate_invoice'] . '"');
		     $p = $p[0];
		     $p->customer = unserialize($p->customer);
		     $customer = $p->customer;
		     $products = unserialize($p->products);
		     $results = new stdClass;
		     $results->id = $p->customer_id;
		     $order_number = $p->order_number;
		     $date = $p->date;
		     $date_end = $p->date_end;
		     $infos = $p->infos;
		     $footer_infos = $p->footer_infos;
		 }elseif(!empty($_GET['convert_quote'])){
			 $p = $bd->get_results('SELECT * FROM ' . $bd->prefix . 'quotes WHERE id="' . $_GET['convert_quote'] . '"');
		     $p = $p[0];
		     $p->customer = unserialize($p->customer);
		     $customer = $p->customer;
		     $products = unserialize($p->products);
		     ...
		     $products = array(array('qty'=>'0','ref'=>$lang['reference'],'description'=>$lang['description'],'price'=>'0','taxes'=>0));
		     $order_number = '00000000';
		     $date = date('d/m/Y');
		     $end = $bd->get_option('default_invoice_term');
...
<div class="customers_box">
	...
	<div class="col-xs-4">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active"><?php echo $lang['new_invoice'];?></li>
          </ol>
        </section>

        <div class="pad margin no-print">
          <div class="callout callout-info" style="margin-bottom: 0!important;">												
            <h4><i class="fa fa-info"></i> <?php echo $lang['new_quote_info_title'];?>:</h4>
            <?php echo $lang['new_quote_info_text'];?>
          </div>
        </div>
       ...
       <!-- Main content -->
        <section class="invoice">
          ...
          <div class="row invoice-info">
            ...
            <div class="col-sm-4 invoice-col">
              <b><?php echo $lang['invoice_number'];?><i style="color:grey;"> <?php echo $lang['save_for_nbr'];?></i></b><br/>
              <br/>
              <b><?php echo $lang['command_number'];?></b> <span class="command_number editable line"><?php echo $order_number; ?></span><br/>
              <b><?php echo $lang['invoice_end'];?>:</b> <span class="editable line date_end"><?php echo $date_end; ?></span><br/>
          ...
          <div class="row no-print">
            <div class="col-xs-12 button_box">
              <form action="edit-invoice.php" method="post" class="form">
	              <?php if(!empty($_GET['convert_quote'])){echo '<input type="hidden" name="convert_quote" value="' . $_GET['convert_quote'] . '" />';} ?>
	            ...
	            <input type="hidden" name="new_invoice" value="true" />
			   ...
			   (function($){
				   ...
				   calcul_total();
			    	...
			    	$(document).on('click','.editable',function(e){
				    	if(!$(e.target).is('textarea') && !$(e.target).is('input') && !$(e.target).is('.wysihtml5-toolbar') && !$(e.target).is('a') && !$(e.target).is('.edit')){
				...
				function addLine(){
					jQuery('<tr><td class="qty"></td><td class="ref"></td><td class="description"></td><td class="price">0</td><td class="taxes"></td><td class="total"></td><td class="delete"><a href="#" class="btn-xs btn-danger"><i class="fa  fa-trash"></i></a></td></tr>').appendTo('.products tbody');
				}
