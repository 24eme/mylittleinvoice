       ...
       <?php
	     ...
	     if(isset($_POST['new_invoice'])){
		     $invoice = array(
			     ...
			     'customer'=>serialize($_POST['customer']),
		     ...
		     if(!empty($_POST['convert_quote'])){
			     ...
			     $bd->update($bd->prefix . 'invoices',array('quote'=>'true','quote_number'=>$_POST['convert_quote']),array('id'=>$_GET['id_invoice']));
			 }
	     }
	     // Enregistrement d'une facture modifiée
	     if(isset($_POST['edit_invoice'])){
		     ...
		     $bd->update($bd->prefix . 'invoices',$invoice,array('id'=>$_POST['edit_invoice']));
		     $_GET['id_invoice'] = $_POST['edit_invoice'];
	     }
	     
	     // Chargement des infos clients si communiqué, ou ajout des infos par défaut
	     if(isset($_GET['id_invoice'])){
		     $results = $bd->get_results('SELECT * FROM ' . $bd->prefix . 'invoices where id="' . $_GET['id_invoice'] . '"');
		     $results = $results[0];
	     ...
	     if(isset($_GET['make_print']) AND isset($_GET['id_invoice'])){
		     echo '<script type="text/javascript">
				    var win = window.open("print-docs.php?id_invoice=' . $_GET['id_invoice'] . '", "_blank");
					if(win){
					    //Browser has allowed it to be opened
					    win.focus();
					}else{
					    jQuery("lol").appendTo(".callout");
					}
				</script>';
	     }
	     ?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <div class="products_type"><h4><?php echo $lang['product_type']; ?></h4>
	        ...
	        <h1>
           <i class="fa fa-file-text"></i> <?php echo $lang['invoice']; ?>
...
<div class="customers_box">
	...
	<div class="col-xs-4">
		        <input type="text" name="search" class="search_cmp search_what form-control" placeholder="Rechercher" class="form-margin" style="height:22px;" />
	        </div>
	        <div class="col-xs-4">
	            <select class="search_where search_cmp form-control col-xs-3" style="height:22px;">
		            ...
		            <option value="name"><?php echo $lang['customer_name']; ?></option>
			...
			<a href="#" class="btn btn-info btn-xs valid_customer" style="float:right;margin-top:5px;"><?php echo $lang['validate']; ?></a>
			<a href="#" class="btn btn-default btn-xs newinvoice" onclick="jQuery('.customers_box').fadeOut();"><?php echo $lang['cancel']; ?></a>
		</div>
          <ol class="breadcrumb">
            <li><a href="index.php"><i class="fa fa-dashboard"></i> <?php echo $lang['home']; ?></a></li>
            <li><a href="invoices.php"><?php echo $lang['invoices']; ?></a></li>
            <li class="active"><?php echo $lang['invoice_number']; ?><?php echo $results->id; ?></li>
          </ol>
        </section>

        <div class="pad margin no-print">
          <div class="callout callout-warning" style="margin-bottom: 0!important;">												
            <h4><i class="fa fa-info"></i> <?php echo $lang['quote_help_title']; ?>:</h4>
            <?php echo $lang['quote_help_text']; ?>
          </div>
       ...
       <!-- Main content -->
        <section class="invoice">
          ...
          <div class="row">
            <div class="col-xs-12">
              <h2 class="page-header">
                ...
                <small class="pull-right"><?php echo $lang['date']; ?>: <span class="editable date"><?php echo $results->date; ?></span></small>
              </h2>
            </div><!-- /.col -->
          </div>
          <!-- info row -->
          <div class="row invoice-info">
            ...
            <div class="col-sm-4 invoice-col">
              <b><?php echo $lang['invoice_number']; ?><i style="color:grey;"><?php echo $results->id; ?></i></b><br/>
              <br/>
              <b><?php echo $lang['command_number']; ?>:</b> <span class="command_number editable line"><?php echo $results->order_number; ?></span><br/>
              <b><?php echo $lang['invoice_end']; ?>:</b> <span class="editable line date_end"><?php echo $results->date_end; ?></span><br/>
              <b><?php echo $lang['customer_number']; ?>:</b> <span class="customer_id"><?php echo $results->customer_id; ?></span>
            </div><!-- /.col -->
          </div><!-- /.row -->
              ...
              <table class="table table-striped products">
                <thead>
                  <tr>
                    ...
                    <th style="width:15%"><?php echo $lang['reference'];?></th>
          ...
          <div class="row">
            ...
            <div class="col-xs-6 infos">
              ...
              <div class="text-muted well well-sm no-shadow" style="margin-top: 10px;">
                <span class="editable invoice_informations text" style="overflow: hidden;display:inline-block;min-height:50px;height: auto;"><?php echo $results->infos; ?></span>        
                </div>
            </div><!-- /.col -->
              ...
              <div class="table-responsive">
                <table class="table">
                  <tr>
                    ...
                    <td class="subtotal"><?php echo number_format($total_products,2); ?><?php echo $bd->get_option('money_symbole'); ?></td>
                  </tr>
                  <tr>
                    <th><?php echo $lang['taxes'];?></th>
                    <td class="total_taxes"><?php echo number_format($total_taxes,2); ?><?php echo $bd->get_option('money_symbole'); ?></td>
                  </tr>
                  <tr>
                    <th><?php echo $lang['total'];?>:</th>
                    <td class="totals"><?php echo number_format($total_products+$total_taxes,2); ?><?php echo $bd->get_option('money_symbole'); ?></td>
                  </tr>
                </table>
              </div>
            </div><!-- /.col -->
          ...
          <div class="row no-print">
            <div class="col-xs-12 button_box">
              <form action="edit-invoice.php?id_invoice=<?php echo $results->id; ?>" method="post" class="form">
             	<a href="#" class="btn btn-default make_print" ><i class="fa fa-print"></i> <?php echo $lang['print'];?></a>
              	<a href="#" class="btn btn-success pull-right valid_form" ><i class="fa fa-hdd-o"></i> <?php echo $lang['save'];?> </a>
              	<!-- Rajouter bouton : Transformer en facture, envoi pdf par mail -->
			  	<a href="#" class="btn btn-primary pull-right make_pdf"><i class="fa fa-download"></i> <?php echo $lang['create_pdf'];?></a>
              	<input type="hidden" name="edit_invoice" id="edit_invoice" value="<?php echo $results->id; ?>" />
			  </form>
              <hr /><span class="invoice_footer editable text" style="overflow: hidden;display:inline-block;min-height:50px;height: auto;"><?php echo $results->footer_infos; ?></span>
            </div>
          </div>
        </section><!-- /.content -->
			   ...
			   (function($){
				   ...
				   calcul_total();
			    	...
			    	$(document).on('click','.editable',function(e){
				    	if(!$(e.target).is('textarea') && !$(e.target).is('input') && !$(e.target).is('.edit')){
			    	...
			    	$(document).on('click','.products td',function(e){
				    	if(!$(e.target).is('textarea') && !$(e.target).is('input')){
					    	...
					    	if($(this).hasClass('total') || $(this).hasClass('delete')){}else{var content = $(this).text();
						    	$(this).html('<input type="text" class="edit" name="edit" style="position:absolute;width:' + $(this).width() + 'px;" value="' + content + '" />');
						    	$(this).children('input').select();
						    	e.stopPropagation(); 
					    	}
					    }
			    	...
			    	// Gestion de la touche "Tab" dans la liste des produits
			    	$(document).on('keydown','.edit',function(event){
				    	if(event.keyCode == '9'){
					    	...
					    	$(objet).html('<input type="text" class="edit" name="edit" style="position:absolute;width:' + $(objet).width() + 'px;" value="' + content + '" />');
			    	...
			    	$('.valid_form').click(function(){
				    	submit_form('edit-invoice.php?id_invoice=' + $('#edit_invoice').val());
				    	return false;
			    	});
			    	$('.make_print').click(function(){
				    	$('.form').attr('target','_blank');
				    	submit_form('print-docs.php?id_invoice=' + $('#edit_invoice').val() + '&make_print=true');
				    	$('.form').attr('target','_self');
				    	return false;
			    	});
			    	$('.make_pdf').click(function(){
				    	$('.form').attr('target','_blank');
				    	submit_form('export-docs.php?id_invoice=' + $('#edit_invoice').val() + '&make_pdf=true');
				    	$('.form').attr('target','_self');
				    	return false;
			    	});
				})(jQuery);
				function addLine(){
					jQuery('<tr><td class="qty">0</td><td class="ref"></td><td class="description"></td><td class="price"></td><td class="taxes"></td><td class="total"></td><td class="delete"><a href="#" class="btn-xs btn-danger"><i class="fa  fa-trash"></i></a></td></tr>').appendTo('.products tbody');
				}
				...
				function submit_form(a){
				    ...
				    $('.customer_infos').each(function(){
					    ...
					    $('<textarea style="display:none;" name="infos">' + $('.invoice_informations').text() + '</textarea>').appendTo('.form');
					    $('<textarea style="display:none;" name="footer_infos">' + $('.invoice_footer').text() + '</textarea>').appendTo('.form');
					    $('.form').get(0).setAttribute('action', a);
					    $('.form').submit();
				}
