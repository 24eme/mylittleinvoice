      ...
      <?php include('include/left.php'); ?>
       ...
       <?php
	     ...
	     if(isset($_POST['edit_quote'])){
		     ...
		     $bd->update($bd->prefix . 'quotes',$quote,array('id'=>$_POST['edit_quote']));
		     $_GET['id_quote'] = $_POST['edit_quote'];
	     }
	     
	     // Chargement des infos clients si communiqué, ou ajout des infos par défaut
	     if(isset($_GET['id_quote'])){
		     $results = $bd->get_results('SELECT * FROM ' . $bd->prefix . 'quotes where id="' . $_GET['id_quote'] . '"');
		     $results = $results[0];
	     ...
	     if(isset($_GET['make_print']) AND isset($_GET['id_quote'])){
		     echo '<script type="text/javascript">
				    var win = window.open("print-docs.php?id_quote=' . $_GET['id_quote'] . '", "_blank");
					if(win){
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <div class="products_type"><h4><?php echo $lang['product_type']; ?></h4>
	        ...
	        <h1>
           <i class="fa fa-file-text-o"></i> <?php echo $lang['quote']; ?> 
...
<a href="new-invoice.php?convert_quote=<?php echo $results->id; ?>" class="btn btn-success btn-xs newinvoice"><i class="fa fa-plus-square"></i> <?php echo $lang['convert_in_invoice']; ?></a>
		</h1>
	...
	<div class="col-xs-4">
	        ...
	        <div class="col-xs-4">
	            <select class="search_where search_cmp form-control col-xs-3" style="height:22px;">
		            ...
		            <option value="id"><?php echo $lang['search_in_nbr_customer']; ?></option>
          ...
          <ol class="breadcrumb">
            ...
            <li class="active"><?php echo $lang['quote']; ?> n°<?php echo $results->id; ?></li>
          </ol>
        </section>
        ...
        <section class="invoice">
          ...
          <div class="row invoice-info">
            ...
            <div class="col-sm-4 invoice-col">
              ...
              <b><?php echo $lang['quote_end']; ?>:</b> <span class="editable line date_end"><?php echo $results->date_end; ?></span><br/>
          ...
          <div class="row no-print">
            <div class="col-xs-12 button_box">
              <form action="edit-quote.php?id_quote=<?php echo $results->id; ?>" method="post" class="form">
             	<a href="#" class="btn btn-default make_print" ><i class="fa fa-print"></i> <?php echo $lang['print']; ?></a>
              	...
              	<input type="hidden" name="edit_quote" id="edit_quote" value="<?php echo $results->id; ?>" />
			  </form>
			   ...
			   (function($){
				   ...
				   calcul_total();
			    	...
			    	$('.valid_form').click(function(){
				    	submit_form('edit-quote.php?id_quote=' + $('#edit_quote').val());
				    	return false;
			    	});
			    	$('.make_print').click(function(){
				    	$('.form').attr('target','_blank');
				    	submit_form('print-docs.php?id_quote=' + $('#edit_quote').val() + '&make_print=true');
				    	$('.form').attr('target','_self');
				    	return false;
			    	});
			    	$('.make_pdf').click(function(){
				    	$('.form').attr('target','_blank');
				    	submit_form('export-docs.php?id_quote=' + $('#edit_quote').val() + '&make_pdf=true');
				    	$('.form').attr('target','_self');
				    	return false;
			    	});
				})(jQuery);
