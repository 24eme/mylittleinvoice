...
if (isset($_GET["blocage"])) {
	...
	$ficbloc=$grc_config['urba']['path']."/blocage/facturation".$annee."-".$bloc_insee.".txt";
...
//	header('Location: facturationat86.php');
    echo "<script type='text/javascript'>document.location.replace('facturationurba.php');</script>";
}
...
if (isset($_GET["unlink"])) {
	...
	if(is_file($grc_config['urba']['path']."/Facturation".$annee."-".$supp_insee.".pdf"))
	{
		unlink($grc_config['urba']['path']."/Facturation".$annee."-".$supp_insee.".pdf");
	}
	$ficlog=$grc_config['urba']['path']."/txt/facturation".$annee."-".$supp_insee.".txt";
	if( file_exists ($ficlog)){
		unlink( $ficlog ) ;
	}
	$ficrecap=$grc_config['urba']['path']."/recap/facturation".$annee."-".$supp_insee.".txt";
	if( file_exists ($ficrecap)){
		unlink( $ficrecap ) ;
	}
	$ficmail=$grc_config['urba']['path']."/mail/mail".$annee."-".$supp_insee.".txt";
...
<?php
	$filename = $grc_config['urba']['path'].'/urbanisme.txt';
 	$ligne= file($filename);
...
while ($i < $num) {
		...
		echo "<td>".$code_adh."</td>";
		echo "<td>".$type."</td>";
		if(is_file($grc_config['urba']['path']."/txt/facturation".$annee."-".$insee.".txt"))
		{
			$ficbloc=$grc_config['urba']['path']."/blocage/facturation".$annee."-".$insee.".txt";
			if( file_exists ($ficbloc)){
				echo "<td>Bloqu&eacute;</td>";
			} else {
				echo "<td><a href=simulationurba.php?id=2&etab=$id name=simul target=_blank><button type=\"button\" class=\"btn btn-xs btn-success\">Simulation</button></a></td>";
			}
			...
			echo "<td align=center><a href=consulturba.php?consult=".$code_adh.">".$annee."</a></td>";
		}
		else
		{
			$ficbloc=$grc_config['urba']['path']."/blocage/facturation".$annee."-".$insee.".txt";
			if( file_exists ($ficbloc)){
				...
				echo "<td><a href=simulationurba.php?id=2&etab=$id name=simul target=_blank><button type=\"button\" class=\"btn btn-xs btn-primary\">Simulation</button></a></td>";
			}
			echo "<td>&nbsp;</td>";
			echo "<td>&nbsp;</td>";
		}
		...
		else
		{
			echo "<td>&nbsp;</td>";
                ...
                {
                        echo "<td align=center><a href=$filenameurba download=$filenameurba>XML</a></td>";
                }
                else
                {
                        echo "<td>&nbsp;</td>";
                }
                $filenamepdf = substr($filenameurba, strpos($filenameurba, $insee."-".$annee."-")+11, strpos($filenameurba, ".xml")-strlen($filenameurba));
                $filenamepdf = $annee."-".$filenamepdf.".pdf";
                unset($filenameurba);
                if(is_file($grc_config['urba']['exportservice_path']."/".$filenamepdf))
                {
                        echo "<td align=center><a href=\"".$grc_config['urba']['exportservice_path']."/$filenamepdf\" download=\"$filenamepdf\">PJ</a></td>";
                }
                else
                {
                        echo "<td>&nbsp;</td>";
...
$('#Modalbloc').on('show.bs.modal', function (event) {
  ...
  $('#myBlocButton').on("click",function(){
		...
		document.location.href='facturationurba.php?blocage='+liensuppr;
	});
})
</script>
<script>
$('#Modalimport').on('show.bs.modal', function (event) {
  ...
  $('#myImportButton').on("click",function(){
		...
		document.location.href='createinvoice.php?import='+liensuppr;
	});
})
</script>
<script>
$('#Modalouinon').on('show.bs.modal', function (event) {
  ...
  $('#myStateButton').on("click",function(){
		...
		document.location.href='facturationurba.php?unlink='+liensuppr;
	});
})
</script>
